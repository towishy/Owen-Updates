...
- installer, archive, package, blockmap, checksum, updater feed URL은 저장하지 않는다.
- 설치 파일은 Microsoft Store 또는 제품별 외부 배포 경로에서 제공한다.

현재 등록 제품: `gsa-dashboard`, [owen-mdbox](owen-mdbox/README.md)
...
- `npm run validate`는 binary/feed artifact나 추가 URL 필드가 들어오면 실패한다.
