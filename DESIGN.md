...
title: "Owen Updates Registry Design Boundary"
status: boundary
product: "owen-updates"
surface: "check-only product update manifest registry"
kind: registry
updated: 2026-08-12
...
# Owen Updates Registry Design Boundary

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | Owen product update checker와 release maintainer |
| Output job | Platform별 latest version을 안전한 check-only JSON으로 제공한다 |
| Primary object | schema v3 update.json manifest |
| Primary action | SemVer와 check-only invariant를 검증해 publish한다 |
| Composition | No UI; machine-readable version contract |
| 핵심 판단 | Installer/feed UI를 만들지 않고 URL 없는 check-only metadata만 제공한다 |

## Context

owen-updates는 MDBOX와 GSA Dashboard가 조회하는 public metadata repository다. 실제 download·install·update prompt는 각 제품이 소유한다.

## Implementation And Evidence

### Implementation

- owen-mdbox/update.json
- gsa-dashboard/update.json
- tools/validate-update-repo.mjs
- tools/sync-platform-releases.mjs

### Evidence

- README.md
- AGENTS.md
- PRODUCT-INTEGRATION-PROMPT.md
- package.json
...
- schemaVersion 3과 platform별 SemVer
- mode check-only
- 명시적 public release opt-in

### Excluded Or App-Owned

- Binary·installer·feed URL
- 이 저장소의 standalone update UI
- 기본 public GitHub Release 생성

## Design Decisions

### D-01. Check-only contract

- **결정**: Manifest는 latest version만 제공하고 install source를 포함하지 않는다
- **이유**: Distribution channel과 update metadata 책임을 분리한다
- **검증**: validate script가 URL과 invalid mode를 거부해야 한다

### D-02. Platform을 명시한다

- **결정**: windows-x64와 mac-arm을 독립 state로 기록한다
- **이유**: 지원하지 않는 architecture에 update를 노출하지 않기 위해서다
- **검증**: Schema와 SemVer test를 실행한다

## State Contract

| 상태 | 표현 | 다음 행동 |
| --- | --- | --- |
| Current | 현재 source·artifact와 owner를 명시한다 | 검증 또는 배포 가능 |
| Invalid | schema·source·artifact 불일치를 구체적으로 표시한다 | 수정 후 재검증 |
| Stale | 원천 제품이나 기준보다 뒤처진 상태를 표시한다 | canonical source와 동기화 |
| Missing | 필수 artifact 부재를 숨기지 않는다 | 생성 또는 owner 확인 |

## Responsive Or Delivery Contract

| 범위 | 유지·변경 | 금지 |
| --- | --- | --- |
| 문서·CLI | 구조화된 text, 명확한 exit state, 복사 가능한 path | 가짜 product UI를 추가하지 않는다 |
| 향후 UI | 실제 output job과 owner가 생긴 뒤 Foundation 후보를 선택 | 현재 artifact repo에 추측성 screen을 만들지 않는다 |
